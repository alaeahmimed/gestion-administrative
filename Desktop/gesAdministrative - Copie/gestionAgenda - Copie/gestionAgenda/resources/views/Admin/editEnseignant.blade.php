        ...
        <div class="col-xl-8">
            <div class="card shadow border-0">
                ...
                <div class="card-body p-5">
                    <form action="{{ route('editEnseignant.update', $enseignant->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('nom') is-invalid @enderror" 
                                    id="nom" name="nom" value="{{ old('nom', $enseignant->user->nom) }}" required>
                                @error('nom')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('prenom') is-invalid @enderror" 
                                    id="prenom" name="prenom" value="{{ old('prenom', $enseignant->user->prenom) }}" required>
                                @error('prenom')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="email" class="form-control @error('login') is-invalid @enderror" 
                                    id="login" name="login" value="{{ old('login', $enseignant->user->login) }}" required>
                                @error('login')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control @error('matiere') is-invalid @enderror" 
                                    id="matiere" name="matiere" value="{{ old('matiere', $enseignant->matiere) }}" required>
                                @error('matiere')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('cycle') is-invalid @enderror" 
                                    ...
                                    <option value="" disabled>Sélectionnez un cycle</option>
                                     <option value="all" {{ old('cycle') == 'all' ? 'selected' : '' }}>Tous les cycles</option> <!-- nouvelle option -->
                                    @foreach($cycles as $cycle)
                                        <option value="{{ $cycle }}" 
                                            {{ old('cycle', $enseignant->cycle) == $cycle ? 'selected' : '' }}>
                                            {{ $cycle }}
                                        </option>
                                    @endforeach
                                </select>
                                        ...
                                        <div class="row" id="classes-container">
                                            @foreach($classesByCycle as $cycle => $classes)
                                                @foreach($classes as $classe)
                                                    @php
                                                        $oldClasses = old('classe', json_decode($enseignant->classe, true) ?? []);
                                                        $isChecked = in_array($classe, $oldClasses);
                                                    @endphp
                                                    <div class="col-md-3 mb-2 class-option" data-cycle="{{ $cycle }}" 
                                                        style="display: {{ $enseignant->cycle == $cycle ? 'block' : 'none' }};">
